...
export function GenerativeArtScene({
  ...
  useEffect(() => {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      vertexShader: `
        ...
        void main() {
          vNormal = normal;
          vPosition = position;
          // Calmer when frugal, more agitated (turbulent) the deeper into the red.
          float amp = 0.10 + (1.0 - uIntensity) * 0.17;
          float d1 = snoise(position * 2.0 + time * 0.5);
          float d2 = snoise(position * 4.3 - time * 0.8) * 0.45;
          float displacement = (d1 + d2) * amp;
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          vec3 finalColor = color * (diffuse * 0.85 + 0.15) + color * fresnel * (0.5 + uIntensity * 0.7);
    ...
    // Additive glow shell — a slightly larger sphere emitting a fresnel rim halo
    // for depth/bloom. Brightens with frugality (uIntensity).
    const glowGeo = new THREE.IcosahedronGeometry(1.2, 20);
    const glowMat = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          float a = fres * (0.22 + uIntensity * 0.4);
    ...
    const render = (t: number) => {
      ...
      glowMat.uniforms.color.value.copy(material.uniforms.color.value);
      const i =
        material.uniforms.uIntensity.value +
        (intensityRef.current - material.uniforms.uIntensity.value) * 0.06;
      material.uniforms.uIntensity.value = i;
      glowMat.uniforms.uIntensity.value = i;
      // Slow breathing pulse for life.
      const pulse = 1 + Math.sin(t * 0.0011) * 0.02;
      mesh.scale.setScalar(pulse);
      glow.scale.setScalar(1.18 * pulse);
      mesh.rotation.y += 0.0005;
      mesh.rotation.x += 0.0002;
      glow.rotation.y -= 0.0003; // counter-rotate the halo for parallax depth
